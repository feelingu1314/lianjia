    ...
    def parse(self, response):
        ...
        if response.request.meta.get('referer', 'other') == '1st':
            ...
            for metro in metros:
                ...
                yield scrapy.Request(url=url, meta={'city': meta, 'metro': 'station'}, callback=self.parse,
                                     dont_filter=True)
        ...
        if house_type in ['普通住宅', '新式里弄', '老公寓', '别墅']:
            ...
            for i, trade_attr_key in enumerate(trade_attr_items, 1):
                trade_attr_value = response.css(
                    '#introduction > div > div > div.transaction > div.content > ul > li:nth-child({}) > span:nth-child(2)::text'.format(
                        i)
                ).extract_first().strip()
            ...
            if hid and rid:
                house_stat_url = 'https://{0}.lianjia.com/ershoufang/housestat?hid={1}&rid={2}'.format(meta, rid,
                                                                                                       hid)  # request时参数错位
                request = scrapy.Request(url=house_stat_url, meta={'city': meta}, callback=self.parse_location,
                                         dont_filter=True)
